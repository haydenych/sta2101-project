...
if (load_model) {
  load("../../output/arterial/light_gbm.RData")
}
```
...
if (!load_model) {
  ...
  for (i in 1:k) {
    train <- slice_arterial(train_val_std[!train_val_std$id %in% cv[[i]], ])
    val <- slice_arterial(train_val_std[train_val_std$id %in% cv[[i]], ])
...
```{r}
save(models, time_model, file="../../output/arterial/light_gbm.RData")
```
...
```{r}
X_train = as.matrix(subset(slice_arterial(train_std), select=c(-tot_crash_count)))
y_train = as.matrix(subset(slice_arterial(train_std), select=c(tot_crash_count)))
...
```{r}
X_test = as.matrix(subset(slice_arterial(test_std), select=c(-tot_crash_count)))
y_test = as.matrix(subset(slice_arterial(test_std), select=c(tot_crash_count)))
