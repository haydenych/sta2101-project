...
if (!load_model) {
  ...
  for (i in 1:k) {
    train <- crash[!crash$id %in% cv[[i]], ]
    val <- crash[crash$id %in% cv[[i]], ]
    
    models[[i]] <- ranger(tot_crash_count~.-id, data=train, num.trees=500, importance="impurity", verbose=T)
  }
...
```{r}
save(models, time_model, file="../../output/all/rf_500.RData")
```
