...
if (load_model) {
  load("../../output/all/light_gbm.RData")
}
```
...
```{r}
save(models, time_model, file="../../output/all/light_gbm.RData")
```
...
```{r}
X_train = as.matrix(subset(train_std, select=c(-tot_crash_count)))
y_train = as.matrix(subset(train_std, select=c(tot_crash_count)))
...
```{r}
X_test = as.matrix(subset(test_std, select=c(-tot_crash_count)))
y_test = as.matrix(subset(test_std, select=c(tot_crash_count)))
...
```{r}
save(model_final, file="../../output/all/light_gbm_final.RData")
```
