...
if (!load_model) {
  start <- Sys.time()
  model <- glm(tot_crash_count~., data=train_std, family=poisson)
  end <- Sys.time()
...
y_pred <- predict(model_step, val_std, type="response")
mae_val <- mean(abs(y_pred - val_std$tot_crash_count))
rmse_val <- sqrt(mean((y_pred - val_std$tot_crash_count) ^ 2))
...
```{r}
save(model, model_step, time_model, time_model_step, file="../../output/all/poisson.RData")
```
